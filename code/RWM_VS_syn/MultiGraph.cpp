...
void MultiGraph::singlelocalCmtyDetection(uint seed, int qlayer, vector<uint> &cmty) {
	cmty.clear();
	...
	if (MC) {
		const int nrolls = 10000;// 4 * graphs[qlayer].nodeSize;
		map<int, int> esreults;
		for (int i = 0; i < nrolls; i++) {
			int length = distribution(generator);
...
/*
void MultiGraph::findCmty(uint qlayer, double **weights, map<uint, double> & prxmts, vector<uint> &cmty) {
	...
	for (uint i = 0; i < nmbofLayer; i++) {
		...
		else if (interFlags[qlayer][i])
			calConductance(graphs[i], conductances[i], sortedxs, interGraphs[qlayer][i]);
...
/*
Multiwalker Model in general multi-network

void MultiGraph::multiWalker(uint seed, uint qlayer, vector<uint> &cmty, double **weights) {
	...
	for (uint i = 0; i < nmbofLayer; i++) {
		if (interFlags[qlayer][i]) {
			multi(xs[i], interGraphs[qlayer][i], xs[qlayer], 1);
			norm(xs[i]);//delete the elements with score less than MINSCORE;
		}
	}
	...
	while (true) {
		...
		for (uint i = 0; i < nmbofLayer; i++) {
			...
			for (uint j = 0; j < nmbofLayer; j++) {
				...
				map<uint, double> xsj2i;
				multi(xsj2i, interGraphs[j][i], xs[j], 1);//go back from layer j to layer i
				switch (wmodel) {
				case MODEL_PC:weights[i][j] += pow(lamda, count)*ParpearsonCorrelation(xs[i], xsj2i, seed, topNodePercent); break;
				case MODEL_COSINE:weights[i][j] += pow(lamda, count)*cosineSimilarity(xs[i], xsj2i, seed, topNodePercent); break;
				case MODEL_EC:weights[i][j] += pow(lamda, count)*edgeCorrectness(graphs[i], xs[i], xsj2i, seed, topNodePercent); break;
				case MODEL_ICS: weights[i][j] += pow(lamda, count)*ICS(graphs[i], xs[i], xsj2i, seed, topNodePercent); break;
				default:
					weights[i][j] += pow(lamda, count)*ParpearsonCorrelation(xs[i], xsj2i, seed, topNodePercent);
				}
			}
			weights[i][i] += pow(lamda, count)*selfsimi;
		}

		if (!mute)
			cout << "pow(lamda , count)\t" << count << "\t" << pow(lamda, count) << endl;
		for (uint i = 0; i < nmbofLayer; i++) {
			double sumi = 0;
			for (uint j = 0; j < nmbofLayer; j++) {
				sumi += weights[i][j];
		...
		for (uint i = 0; i < nmbofLayer; i++) {
			...
			for (uint j = 0; j < nmbofLayer; j++) {
				...
				map<uint, double>temp1, temp2;
				if (i != j) {
					multi(temp1, interGraphs[i][j], xs[i], 1);//from layer i to layer j
															  //multi(temp2, graphs[j], temp1, weights[i][j]);//one step in layer j
															  //multi(temp1, interGraphs[j][i], temp2, 1);//go back from j to i
					multi(temp2, graphs[j], temp1, beta);//one step RWR in layer j
					addArray(temp2, temp1, 1 - beta);//one step RWR in layer j
					multi(temp1, interGraphs[j][i], temp2, weights[i][j]);//go back from j to i
				}
				else
					multi(temp1, graphs[j], xs[i], weights[i][j]);//one step in layer j
				double sum = sumArray(temp1);
				addArray(nxs[i], temp1, alpha);
			}
			addArray(nxs[i], vs[i], (1 - alpha));
			norm(nxs[i]);
	...
	for (uint i = 0; i < nmbofLayer; i++) {
		map<uint, double> &x = xs[i];
		for (map<uint, double>::iterator vite = x.begin(); vite != x.end(); vite++) {
			uint degree = graphs[i].degree(vite->first);
			if (degree != 0)
				vite->second /= degree;
		}
	}
	...
	//here I only focus on the qlayer.
	findCmty(qlayer, weights, cmty);
}
*/